-- ========================================
-- 销售数据迁移脚本 V3
-- 修正：original_id 改为 INTEGER 类型存储
-- ========================================

-- 0. 首先修改 sales_optimized 表的 original_id 字段类型
ALTER TABLE sales_optimized 
ALTER COLUMN original_id TYPE INTEGER USING original_id::INTEGER;
...
SELECT 
    ...
    id as original_id  -- 原表的INTEGER ID
FROM primary_sales;
...
WITH primary_mapping AS (
    SELECT 
        ...
        so.sales_code,
        so.wechat_name,
        so.original_id as old_id
    FROM sales_optimized so
    WHERE so.original_table = 'primary_sales'
)
INSERT INTO sales_optimized (
    -- 主键使用UUID
    id,
...
SELECT 
    ...
    -- 关系链（使用新表的UUID）
    pm.new_uuid as parent_sales_id,  -- 使用映射后的UUID
    pm.sales_code as parent_sales_code,
    pm.wechat_name as parent_sales_name,
    ...
    s.id as original_id  -- 原表的INTEGER ID
FROM secondary_sales s
LEFT JOIN primary_mapping pm ON s.primary_sales_id = pm.old_id;
