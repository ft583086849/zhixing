...
async function getDatabaseConfig() {
  ...
  // 尝试从Vercel API获取配置
  try {
    console.log('📋 尝试从Vercel获取数据库配置...');
    const response = await fetch('https://zhixing-seven.vercel.app/api/health');
    const healthData = await response.json();
    
    if (healthData.success && healthData.data.database.connected) {
      console.log('✅ 从Vercel健康检查确认数据库连接正常');
      
      // 使用默认的PlanetScale配置
      return {
        host: 'aws.connect.psdb.cloud',
        user: 'ft583086849', // 从现有脚本看到的用户名
        password: process.env.PLANETSCALE_PASSWORD || '需要设置密码',
        database: 'zhixing',
        ssl: {
          rejectUnauthorized: false
        }
    ...
    console.log('⚠️ 无法从Vercel获取配置:', error.message);
  }
  
  throw new Error('无法获取数据库配置，请设置环境变量或PLANETSCALE_PASSWORD');
}
...
async function testDatabaseSchema() {
  ...
  try {
    ...
    if (error.message.includes('密码')) {
      console.log('\n💡 解决方案:');
      console.log('1. 设置环境变量 PLANETSCALE_PASSWORD');
      console.log('2. 或者设置完整的数据库环境变量:');
      console.log('   - DB_HOST');
      console.log('   - DB_USER');
