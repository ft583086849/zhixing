...
export class StatsUpdater {
  ...
  static async updateStats(period = 'all', startDate = null, endDate = null) {
    ...
    try {
      ...
      // 1. 获取订单数据
      let ordersQuery = supabase.from('orders_optimized').select('*');
      ...
      // 🔧 新增：获取sales_optimized表的实际佣金数据
      const { data: salesOptimized, error: salesError } = await supabase
        .from('sales_optimized')
        .select('total_commission, paid_commission');
      if (salesError) console.warn('获取销售佣金数据失败:', salesError);
      
      // 3. 计算统计数据
      const stats = this.calculateStats(orders, primarySales || [], secondarySales || [], period, salesOptimized || []);
   ...
   * 计算统计数据
   */
  static calculateStats(orders, primarySales, secondarySales, period, salesOptimized = []) {
    // 订单状态统计
    ...
    return {
      ...
      paid_commission: this.calculateActualPaidCommission(salesOptimized).toFixed(2), // 🔧 修复：使用实际已返佣金
      pending_commission: this.calculateActualPendingCommission(salesOptimized).toFixed(2), // 🔧 修复：使用实际待返佣金
   ...
   * 计算实际已返佣金
   */
  static calculateActualPaidCommission(salesOptimized) {
    if (!Array.isArray(salesOptimized) || salesOptimized.length === 0) {
      return 0;
    }
    
    return salesOptimized.reduce((total, sale) => {
   ...
   * 计算实际待返佣金
   */
  static calculateActualPendingCommission(salesOptimized) {
    if (!Array.isArray(salesOptimized) || salesOptimized.length === 0) {
      return 0;
    }
